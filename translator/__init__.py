...
def auto_translator(msg: str):
    return translator.translate(msg, dest=config.secondary_language).text if translator.detect(msg).lang == config.global_language else translator.translate(msg, dest=config.global_language).text
...
def on_load(server: PluginServerInterface, old):
    ...
    server_inst = server
    load_config(None)
    help_message = f'Use t and space, then what you want to translate, {config.global_language.upper()} to {config.secondary_language.upper()} & ALL LANGUAGES to {config.global_language.upper()} auto'
